...
//     // Pomocnik sprawdzający, czy jesteśmy w ciele binarnym
...
//     pub fn point(self: &Rc<Self>, x: FieldElement, y: FieldElement) -> Option<ECPoint> {
//         if self.is_binary() {
//             // Równanie dla F_2^m: y^2 + xy = x^3 + ax^2 + b
//             let y2 = y.pow(&BigInt::from(2));
...
//             }
//         } else {
//             // Równanie Weierstrassa dla p > 3: y^2 = x^3 + ax + b
//             let y2 = y.pow(&BigInt::from(2));
//             let x3 = x.pow(&BigInt::from(3));
//             let ax = self.a.clone() * x.clone();
//             let rhs = x3 + ax + self.b.clone();
...
//                 // --- Logika dla ciał binarnych F_2^m ---
//                 if curve.is_binary() {
//                     if x1.coeffs == x2.coeffs {
//                         if y1.coeffs == y2.coeffs {
//                             // 1. Podwajanie (Doubling) P + P w F_2^m
//                             // lambda = x1 + y1/x1
//                             // x3 = lambda^2 + lambda + a
//                             // y3 = x1^2 + (lambda + 1)x3

//                             // Jeśli x1 == 0, to P = -P (bo -P = (0, 0+y) = P). Wtedy 2P = O.
...
//                         } else {
//                             // 2. P + (-P) = O. W F_2^m, jeśli x1=x2 i y1!=y2, to są odwrotności.
//                             return ECPoint::Infinity { curve };
//                         }
//                     } else {
//                         // 3. Dodawanie (Addition) P != Q w F_2^m
//                         // lambda = (y1 + y2) / (x1 + x2)
...
//                 // --- Logika dla Short Weierstrass (F_p, p > 3) ---
//                 else {
//                     if x1.coeffs == x2.coeffs && y1.coeffs != y2.coeffs {
//                         return ECPoint::Infinity { curve };
//                     }
...
pub mod serialization; // <--- Rejestrujemy nowy moduł (plik)
...
pub use serialization::FieldSerde; // <--- Eksportujemy Trait, żeby był widoczny