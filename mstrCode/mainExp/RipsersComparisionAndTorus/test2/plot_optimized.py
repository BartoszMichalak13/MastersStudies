...
if __name__ == "__main__":
    ...
    # Plot RAM Usage - tylko wersje z progiem
    ...
    ax1.set_title("Memory Usage (Optimized with Threshold=2.0)")
    ax1.set_xlabel("Number of Points (N)")
    ax1.set_ylabel("Peak RAM (MB)")
    ax1.grid(True, linestyle='--', alpha=0.7)
    ax1.legend()

    # Plot Time Usage - tylko wersje z progiem
    ...
    ax2.set_title("Execution Time (Optimized with Threshold=2.0)")
    ax2.set_xlabel("Number of Points (N)")
    ax2.set_ylabel("Time (seconds)")
    ax2.grid(True, linestyle='--', alpha=0.7)
    ax2.legend()

    plt.suptitle("Benchmark Performance: Optimized Threshold Approaches", fontsize=16)
    plt.tight_layout()
    plt.savefig("../plots/optimized_benchmark_plot_giotto.png", bbox_inches='tight')
    print("Zapisano wykres porównawczy: 'optimized_benchmark_plot.png'.")